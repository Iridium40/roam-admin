...
export function AdminLayout({
    ...
    const fetchAdminUser = async () => {
      if (user?.id) {
        try {
          ...
          if (error) {
            console.log("Admin user fetch failed - using email fallback");
            return;
          }
          ...
          console.log("Admin user fetch error - using email fallback");
        }
      }
    };
  ...
  // Toggle sidebar collapsed state and save to settings
  const toggleSidebarCollapse = async () => {
    ...
    try {
      const settings = JSON.parse(savedSettings);
      settings.sidebar_collapsed = newCollapsedState;
    ...
    if (user?.id) {
      try {
        ...
        // Database save failed, but localStorage is updated
        console.log("Failed to save sidebar setting to database:", err);
      }
    }
  };
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 bg-white border-r border-sidebar-border transition-all duration-300 ease-in-out lg:translate-x-0",
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-center p-6 border-b border-sidebar-border">
            <ROAMLogo size="sm" showText={!sidebarCollapsed} />
          </div>
